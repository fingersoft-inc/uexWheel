  ...
  $(document).ready(function(){
    ...
    $(document)
      ...
      .on(touchMove, function(e){
        ...
        touch.el && touch.el.trigger('swipeMove'+(swipeDirection(touch.x1, touch.x2, touch.y1, touch.y2)),{dx:deltaX,dy:deltaY});
      })
      ...
      var httpId = getXmlHttpId();
        ...
        var mime = settings.accepts[dataType],
            ...
            addAppVerify = function(settings){
                if(settings.appVerify === true){
                    //添加app 认证头
                    xhr.setAppVerify(httpId,1);
                }
                if(settings.appVerify === false){
                    //添加app 认证头
                    xhr.setAppVerify(httpId,0);
                }
            },
            //更新证书信息
        ...
        if(settings.data && settings.contentType === false){
            for(name in settings.data){
                if(appcan.isString(settings.data[name])){
                    //添加普通数据
...
window.appcan && appcan.define('window',function($,exports,module){
    ...
    */
    ...
    function popoverElement(id,url,left,top,name){
        var argObj = null;
        ...
        openPopover(name,0,url,'',left,top,width,height,fontSize,0,0);
    }
...
window.appcan && appcan.define('frame',function($,exports,module){
    ...
    module.exports = {
        ...
        open:function(id,url,left,top,name,index,change){
            var argObj = null;
            if(arguments.length === 1 && appcan.isPlainObject(id)){
                argObj = id;
                id = argObj['id'] || 0;
            ...
            if(appcan.isArray(url)){
                ...
                appWin.popoverElement(id,url,left,top,name);
            }
        },
